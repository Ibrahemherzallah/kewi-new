...
interface LoyaltyContextType {
  points: number;
  addPoints: (purchaseAmount: number) => number; // ✅ returns earned points
  getDiscount: () => {
    percentage: number;
...
const calculateDiscount = (
    points: number
): { percentage: number; type: "discount" | "free_product" | "none" } => {
  if (points >= 100) {
...
export const LoyaltyProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const addPoints = useCallback((purchaseAmount: number) => {
    ...
    return earnedPoints; // ✅ keep returning
  }, []);
  ...
  const getNextMilestone = useCallback(
      (): { pointsNeeded: number; reward: string } | null => {
        if (points >= 100) {
        ...
        return {
          ...
          reward: `${currentDiscount + 5}% discount`,
        };
      },
      [points]
  );
