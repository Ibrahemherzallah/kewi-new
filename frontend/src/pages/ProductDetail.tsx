...
const ProductDetail = () => {
  ...
  // const totalStock = product?.stockNumber ?? 0;
  const badgeEn = product?.isSoldOut ? 'sold out' : product?.isOnSale ? 'on sale' : product?.isSoon ? 'coming soon' : 'available' ;
  const badgeAr = product?.isSoldOut ? 'نفذ' : product?.isOnSale ? 'معروض للبيع' : product?.isSoon ? 'قريباً' : 'متوفر' ;

  const price = product?.customerPrice ?? 0;
...
// Stock number: if variant → use its stock
  ...
  const handleAddToCart = (prod?: ApiProduct) => {
    ...
    toast({
      ...
      description: `${qty}x ${getLocalizedName(
          productToAdd
      )} ${
          language === "ar"
  ...
  console.log("aaaaaaaaaaaaaa product is: " , product)
  console.log("aaaaaaaaaaaaaa selectedImage is: " , selectedImage)
  if (loading) {
    return (
              ...
              {isMulti && variants.length > 0 && (
                  <div className="space-y-3">
                    <div className="text-sm text-muted-foreground">
                      {language === "ar" ? "الألوان المتوفرة" : "Available Colors"}
                    </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button
                      ...
                      onClick={() =>
                          setQuantity(
                              totalStock > 0
                                  ? Math.min(totalStock, quantity + 1)
                                  : quantity + 1
                          )
                      }
                  >
